...
DEST_PHOTOS = {
    ...
    "Vienne":         "https://images.unsplash.com/photo-1516550135131-field?w=800&q=80",
    ...
    "Kotor":          "https://images.unsplash.com/photo-1555990538-1732b0258235?w=800&q=80",
    "Malte":          "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80",
...
<div style="background:var(--white);border:1px solid var(--border);
            padding:1.2rem 1.4rem 0.8rem;margin-bottom:1.5rem;">
""", unsafe_allow_html=True)
    ...
    with row1[2]:
        d2 = st.date_input(T["dates"] + " ↙", value=date.today() + timedelta(days=37),
                           min_value=date.today())
    ...
    with row1[6]:
        budget = st.number_input(T["budget"], min_value=500, max_value=50000,
                                 value=3000, step=100)

    row2 = st.columns([4, 1, 1])
    with row2[0]:
        st.markdown(f'<div style="font-size:0.72rem;letter-spacing:0.06em;text-transform:uppercase;color:#6B6560;margin-bottom:3px">{T["fav"]} (optionnel)</div>', unsafe_allow_html=True)
...
<div class="dest-card" style="cursor:default;">
  <div class="photo-wrap">
    <img class="dest-photo" src="{photo}" alt="{nom}" loading="lazy" style="height:180px;">
  </div>
  <div class="dest-body" style="padding:0.7rem 1rem;">
    <div class="dest-name" style="font-size:1.1rem;">{flag} {nom}</div>
    <div class="dest-country">{pays}</div>
  </div>
</div>
""", unsafe_allow_html=True)
        st.markdown(f'<div style="text-align:center;font-size:0.78rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--muted);margin-top:1rem;">{T["fill_form"]}</div>', unsafe_allow_html=True)
...
# Coordonnées GPS pour la carte backpacker
DEST_COORDS = {
    ...
    "Koh Samui":(9.5120,100.0136),"Hanoï":(21.0285,105.8542),"Ho Chi Minh":(10.8231,106.6297),
    "Hội An":(15.8801,108.3380),"Da Nang":(16.0544,108.2022),"Siem Reap":(13.3633,103.8564),
    "Bagan":(21.1717,94.8585),"Luang Prabang":(19.8845,102.1348),"Kuala Lumpur":(3.1390,101.6869),
    "Singapour":(1.3521,103.8198),"Bali":(8.4095,-115.1889),"Lombok":(-8.6500,116.3240),
    "Palawan":(9.8349,118.7384),"Goa":(15.2993,74.1240),"Jaipur":(26.9124,75.7873),
    "Kerala":(10.8505,76.2711),"Katmandou":(27.7172,85.3240),"Malé":(4.1755,73.5093),
    "Pékin":(39.9042,116.4074),"Shanghai":(31.2304,121.4737),"Séoul":(37.5665,126.9780),
    "Dubaï":(25.2048,55.2708),"Pétra":(30.3285,35.4444),"Tel Aviv":(32.0853,34.7818),
    "Tbilissi":(41.6938,44.8015),"Samarcande":(39.6542,66.9597),"Doha":(25.2854,51.5310),
    ...
    "Stockholm":(59.3293,18.0686),"Copenhague":(55.6761,12.5683),"Oslo":(59.9139,10.7522),
    "Reykjavik":(64.1466,-21.9426),"Prague":(50.0755,14.4378),"Cracovie":(50.0647,19.9450),
    "Istanbul":(41.0082,28.9784),"Cappadoce":(38.6431,34.8289),"Londres":(51.5074,-0.1278),
    "Édimbourg":(55.9533,-3.1883),"Dublin":(53.3498,-6.2603),
    "New York":(40.7128,-74.0060),"Los Angeles":(34.0522,-118.2437),"Miami":(25.7617,-80.1918),
    "San Francisco":(37.7749,-122.4194),"Las Vegas":(36.1699,-115.1398),"Hawaii":(21.3069,-157.8583),
    "Montréal":(45.5017,-73.5673),"Vancouver":(49.2827,-123.1207),"Toronto":(43.6532,-79.3832),
    "Mexico":(19.4326,-99.1332),"Cancún":(21.1619,-86.8515),"La Havane":(23.1136,-82.3666),
    "Carthagène":(10.3910,-75.4794),"Medellín":(6.2442,-75.5812),"Bogotá":(4.7110,-74.0721),
    "Cusco":(-13.5319,-71.9675),"Lima":(-12.0464,-77.0428),"La Paz":(-16.5000,-68.1500),
    "Rio de Janeiro":(-22.9068,-43.1729),"Buenos Aires":(-34.6037,-58.3816),
    "Patagonie":(-54.8019,-68.3030),"Santiago":(-33.4489,-70.6693),"Atacama":(-22.9087,-68.2000),
    "Galápagos":(-0.9538,-90.9656),"Marrakech":(31.6295,-7.9811),"Fès":(34.0181,-5.0078),
    "Essaouira":(31.5085,-9.7595),"Le Caire":(30.0444,31.2357),"Louxor":(25.6872,32.6396),
    "Zanzibar":(-6.1659,39.2026),"Serengeti":(-2.3333,34.8333),"Nairobi":(-1.2921,36.8219),
    "Le Cap":(-33.9249,18.4241),"Victoria Falls":(-17.9243,25.8572),"Mahé":(-4.6191,55.4513),
    "Île Maurice":(-20.3484,57.5522),"Dakar":(14.7167,-17.4677),
    "Sydney":(-33.8688,151.2093),"Melbourne":(-37.8136,144.9631),"Cairns":(-16.9186,145.7781),
...
""", unsafe_allow_html=True)
            ...
            st.markdown(f"""
<div style="font-family:'Cormorant Garamond',serif;font-size:1.4rem;color:var(--ink);
            margin-bottom:0.8rem;">Carte de l'itinéraire</div>
""", unsafe_allow_html=True)
            try:
                import folium
                ...
                first = itinerary[0]["dest"]
                first_coords = DEST_COORDS.get(first["nom"], (20, 10))
                m = folium.Map(
                ...
                for step_idx, step in enumerate(itinerary):
                    dest = step["dest"]
                    coords = DEST_COORDS.get(dest["nom"])
                    if not coords:
                        continue
                    ...
                    folium.Marker(
                        location=coords,
                        popup=folium.Popup(
                            f"<b>{dest['flag']} {dest['nom']}</b><br>"
                            f"{dest['pays']}<br>"
                            f"{step['nuits']} nuits<br>"
                            f"~{step['subtot']:,} €",
                            max_width=200
                        ),
                        tooltip=f"{step_idx+1}. {dest['nom']}",
                        icon=folium.DivIcon(
                            html=f"""<div style="
                                ...
                                box-shadow:0 2px 6px rgba(0,0,0,0.3);">{step_idx+1}</div>""",
                            icon_size=(28, 28),
                            icon_anchor=(14, 14),
                        )
                    ).add_to(m)
