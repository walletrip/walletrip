...
# Formulaire utilisateur (La case destination a bien été supprimée !)
...
if submit_button:
    ...
    if nb_jours <= 0:
        ...
        # Catalogue de destinations couvrant le monde entier (Europe, Afrique, Asie, Amérique)
        destinations_data = {
        ...
        for ville, infos in destinations_data.items():
            # Calculs budgétaires proportionnels
            ...
            if total_estime <= budget:
                ...
                # Encodage propre du nom de la ville pour le lien Booking
                city_encoded = urllib.parse.quote(infos["query"])
                
                # LIENS EN DUR AVEC INTÉGRATION DE VOTRE TRACEUR D'AFFILIATION 531779
